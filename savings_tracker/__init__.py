...
def main(savings_file: pathlib.Path) -> None:
    ...
    for i, goal in enumerate(savings_data['goals']):
        ...
        if start_date > spend_date:
            logger.warning('start date of %s is greater than spend date of %s for item %s', start_date, spend_date,
                           goal.get('name', i))
            continue
    ...
    print('┃ {} ┃ Start Date ┃  End Date  ┃ Percentage ┃ {} ┃ {} ┃'.format(
    ...
    for goal in goals:
        print('│ {} │ {} │ {} │ {:10} │ ${} │ ${} │'.format(
            ...
            str(goal.target_savings).ljust(max(amount_len, len('Amount Saved') - 1)),
        ))
    ...
    if savings_data.get('base', 0):
        print('│ {} │            │            │            │ {} │ ${} │'.format(
            ...
            ' ' * max(amount_len + 1, len('Goal')),
            str(savings_data['base']).ljust(max(amount_len, len('Amount Saved') - 1)),
        ))
...
def cli() -> int:
    ...
    parser.add_argument('--file', required=True, type=pathlib.Path, help='Path to JSON file containing savings goals')
